...
public class PickedDAO {
	...
	public void pickDelete(String session_id, int picked_board_num) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String pickdel = "DELETE FROM picked WHERE picked_id = ? AND picked_board_num = ?";
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(pickdel);
			pstmt.setString(1, session_id);
			pstmt.setInt(2, picked_board_num);
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		}
